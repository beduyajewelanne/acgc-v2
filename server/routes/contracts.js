...
contractRoutes.post("/api/get_user_contracts", async (req, res) => {
    ...
    try {
        checkAuth(token, _id, async (isValid) => {
            ...
            const result = await get_data_helper("contracts", query);
            if (result?.payload?.length > 0) {
                // await actionLog(_id, "Viewed contracts");
                return res.status(200).json({ remarks: "success", message: "Contracts retrieved successfully", payload: result.payload });
            } else {
                return res.status(200).json({ remarks: "success", message: "No contracts found", payload: [] });
            }
        });
    } catch (err) {
